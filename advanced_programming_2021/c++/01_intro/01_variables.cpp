#include <cmath>    // M_PI
#include <iomanip>  // std::setprecision  (for expr as hex, oct, dec)
#include <iostream>
#include <string>
...
int main() {
  ...
  std::cout << "on my machine the size of a bool is " << sizeof(bool)
            << " bytes\n";  //it is 8 bits for memory alignement when defined 
  ...
  // not built-in but useful :). To avoid the C problems of strings
  ...
  // std::cout << var++;  // allowed and prevedible operation 
  // std::cout << ++var;  // allowed and prevedible operation 
  std::cout << var;
  
  // std::cout << ++var << var++;  // UNDEFINED BEHAVIOR, DO NOT DO THAT!!!!! 
  ...
  x = 1 << 3;   // 8  left shift
  x = 16 >> 2;  // 4  right shift
  //OSS: if you go out the bits dedicated to that type you obtain zeros for sure
  ...
  test = a >= b;
  test = a != b;
  test = a =! b; //here you are assigning the negation of b to a, it is not a boolean 			    operation
  
  a = b = c = 0; //all == 0, the = "return" something
